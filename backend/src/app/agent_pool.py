...
class AgentPool:
    ...
    def preload_singletons(self) -> None:
        ...
        """
        singleton_configs = [
            config for config in self._registry.list_agents() if config.scope == "singleton"
        ]
        ...
        unregistered = [c for c in singleton_configs if c.agent_id not in self._singletons]

        if not unregistered:
            logger.info(f"All singleton agents already registered: {list(self._singletons.keys())}")
            return

        logger.info(f"Preloading {len(unregistered)} unregistered singleton agent(s)...")
        for config in unregistered:
            try:
                ...
                logger.error(f"Failed to create agent {config.agent_id}: {e}", exc_info=True)
                raise RuntimeError(f"Failed to preload singleton agent {config.agent_id}") from e
