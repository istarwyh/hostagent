...
本项目采用清晰的分层架构，确保职责分离和代码可维护性。

### 13.1 架构概览

```
backend/src/
├── facade/          # API 层：HTTP 端点定义，请求/响应处理
├── app/             # 应用层：业务流程编排，事务管理
├── service/         # 服务层：核心业务逻辑，Agent 定义
├── repository/      # 数据层：数据持久化，checkpointer 管理
├── model/           # 模型层：Pydantic 数据模型定义
├── client/          # 客户端层：外部服务调用封装
└── util/            # 工具层：日志、配置等通用工具
```
...
| 层级 | 职责 | 示例 |
|------|------|------|
| **Facade** | HTTP 端点、请求验证、响应格式化、SSE 流处理 | `research_agent_api.py` |
| **App** | 业务流程编排、跨服务协调、事务边界 | 复杂业务场景的编排逻辑 |
| **Service** | Agent 定义、业务规则、工具注册 | `research_agent.py` |
| **Repository** | Checkpointer 管理、状态持久化 | `checkpointer.py` |
| **Model** | 请求/响应模型、领域对象 | `ResearchRequest`, `StreamEvent` |
| **Client** | 外部 API 调用、LLM 客户端 | OpenAI/Anthropic 客户端封装 |
| **Util** | 日志、配置、通用工具函数 | `logger.py` |
...
**关键原则**：
- 上层可以依赖下层，下层不能依赖上层
- `Util` 是基础设施层，所有层都可以依赖
- `Model` 是纯数据定义，不包含业务逻辑
...
```python
# backend/src/facade/research_agent_api.py

from fastapi import FastAPI, HTTPException
from fastapi.responses import StreamingResponse

from src.service.research_agent.research_agent import agent
...
```python
# backend/src/repository/checkpointer.py
...
logger.error("发生错误", exc_info=True)  # 包含堆栈信息
```
...
| 层级 | 日志级别 | 记录内容 |
|------|----------|----------|
| **Facade** | INFO/ERROR | 请求入口、响应状态、异常捕获 |
| **App** | INFO | 业务流程关键节点、事务边界 |
| **Service** | INFO/DEBUG | Agent 执行状态、工具调用、业务决策 |
| **Repository** | DEBUG | 数据读写操作、checkpoint 状态 |
...
logger.error(f"Failed to process request: {thread_id}", exc_info=True)

# ❌ 错误：在循环中大量记录
for item in large_list:
    logger.debug(f"Processing {item}")  # 避免
...
├── error.log            # 仅错误日志
├── daily.log            # 按日期轮转（保留7天）
└── structured/
    └── structured.jsonl # JSON 格式结构化日志
```

---

## 15. Effective Python 最佳实践
...
# ✅ 函数不超过 30 行，复杂逻辑拆分
def process_checkpoint(checkpoint: dict) -> dict:
    channel_values = extract_channel_values(checkpoint)
    messages = extract_messages(channel_values)
    return {"messages": messages, **extract_metadata(channel_values)}
```
...
class AgentExecutionError(Exception):
    ...
    def __init__(self, message: str, thread_id: str):
        ...
        super().__init__(f"[{thread_id}] {message}")

# ✅ 异常处理与日志结合
async def execute_stream_run(thread_id: str, input_data: dict):
    try:
        async for chunk in agent.astream(input_data, config=config):
            yield chunk
    except Exception as e:
        logger.error(f"Stream execution failed: {thread_id}", exc_info=True)
        raise AgentExecutionError(str(e), thread_id) from e
```
...
async def invoke_research(request: ResearchRequest) -> ResearchResponse:
    config = {"configurable": {"thread_id": request.thread_id}}
    result = await agent.ainvoke(
        {"messages": [{"role": "user", "content": request.query}]},
        config=config
    )
    return ResearchResponse(result=result, thread_id=request.thread_id)
...
async def event_generator(
    ...
    async for event in agent.astream_events(input_data, version="v2"):
        yield format_sse_event(event, thread_id)
    yield format_sse_event({"event": "done"}, thread_id)
```
...
agent = create_research_agent(tools, instructions, checkpointer)

# 测试环境
test_checkpointer = MemorySaver()
test_agent = create_research_agent(tools, instructions, test_checkpointer)
```
